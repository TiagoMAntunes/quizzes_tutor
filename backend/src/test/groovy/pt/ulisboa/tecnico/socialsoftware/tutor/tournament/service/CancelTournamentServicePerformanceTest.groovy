...
@DataJpaTest
class CancelTournamentServicePerformanceTest extends Specification {
    ...
    def "performance testing to cancel 750K tournaments"() {
        given: "750K cancelable tournaments"
        1.upto(N_TOURNAMENTS, {tournamentService.createTournament(tournamentDto, courseExecution, user_id)})

        when: //Assumes tournaments ids go from 1 up to N_TOURNAMENTS otherwise all ids would have to be stored in memory
        1.upto(N_TOURNAMENTS, {tournament_id -> tournamentService.cancelTournament(tournament_id, user_id)})
